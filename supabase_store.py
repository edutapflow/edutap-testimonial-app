...
def get_blank_template_reference(feedback_type: str) -> Optional[Dict[str, Any]]:
    """Return a blank/existing template from the templates bucket/local assets for upload guidance.

    This intentionally uses template images, not generated testimonial images.
    """
    t = normalize_type(feedback_type)
    candidates: List[str] = []
    ...
    if t == "edutap":
        ...
        preferred = [
            "Mentor feedback.jpg",
            "Mentor Feedback.jpg",
            "About Our Mentor.jpg",
            "Sample.jpg",
            "sample.jpg",
        ]
    elif t == "support":
        folder = "Support Feedback"
        preferred = ["Support Feedback.jpg", "Support Feedback.jpeg", "Support Feedback.png"]
    else:
        return None

    candidates.extend([f"{folder}/{name}" for name in preferred])

    # If there is no generic blank file, use the first existing template in that folder as a reference.
