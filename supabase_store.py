...
def delete_records(record_ids: Iterable[int], delete_images: bool = True) -> Dict[str, Any]:
    """Delete selected testimonial rows and, when possible, their generated image files.

    Uses the service-role Supabase client already configured for this app.
    If storage cleanup fails for a file, the DB row is still deleted; the returned
    payload includes those storage errors for debugging.
    """
    ids: List[int] = []
    seen = set()
    ...
    if not ids:
        return {"deleted_count": 0, "deleted_ids": [], "storage_errors": []}

    client = get_supabase_client()

    # Fetch image paths first so we can remove files from generated-images.
    rows: List[Dict[str, Any]] = []
    try:
        res = client.table(TABLE_NAME).select("id,image_path").in_("id", ids).execute()
        rows = res.data or []
    except Exception:
        rows = []
    ...
    if delete_images:
        ...
        # Supabase storage remove works with a list. Chunk to avoid very large requests.
        for i in range(0, len(paths), 100):
            chunk = paths[i:i + 100]
            try:
                client.storage.from_(GENERATED_BUCKET).remove(chunk)
            except Exception as exc:
                storage_errors.append({"paths": ", ".join(chunk), "error": str(exc)})

    deleted_count = 0
    # Delete DB rows. Chunk IDs for safety.
    for i in range(0, len(ids), 100):
        chunk = ids[i:i + 100]
        res = client.table(TABLE_NAME).delete().in_("id", chunk).execute()
        deleted_count += len(res.data or chunk)
...
def build_zip_for_records(records: Iterable[Dict[str, Any]]) -> bytes:
    """Build a ZIP for selected records without duplicate ZIP entry names."""
    buf = BytesIO()
    used_names: set[str] = set()

    def unique_zip_name(base_name: str, record_id: Any) -> str:
        base_name = os.path.basename(str(base_name or "").strip()) or f"testimonial_{record_id}.png"
        ...
        rid_part = _safe_path_part(str(record_id or "record"))
        candidate = f"{stem}_{rid_part}{ext}"
        counter = 2
        while candidate in used_names:
            candidate = f"{stem}_{rid_part}_{counter}{ext}"
            counter += 1
        used_names.add(candidate)
        return candidate
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for row in records:
            ...
            filename = unique_zip_name(filename, row.get("id"))
            try:
...
def expected_template_relpaths(feedback_type: str, data: Optional[Dict[str, Any]] = None) -> List[str]:
    """Expected fixed PNG design template for each feedback type."""
    t = normalize_type(feedback_type)
    mapping = {
        "edutap": "NewDesign/EduTap.png",
        "course": "NewDesign/Course.png",
        "event": "NewDesign/Event.png",
        "mentor": "NewDesign/Mentor.png",
        "support": "NewDesign/Support.png",
    }
    rel = mapping.get(t, "NewDesign/EduTap.png")
    return [rel]
