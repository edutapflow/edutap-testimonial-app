...
- greetings/salutations/sign-offs that address a person by name (e.g. "Dear Rahul", "Hi Priya", "Thanks, Anita"),
...
- operational requests about payment links, recordings, login issues, scheduling, or follow-up actions,
...
- the "On [date] Feedback <feedback@edutap.co.in> wrote:" quoted-reply header line.
...
- the student's primary feedback sentence — the core message of the testimonial must always stay visible,
- any sentence that is a direct student opinion, compliment, praise, or gratitude (e.g. "Thank you for being so proactive", "It indeed helped me a lot", "The support was excellent", "You resolved my issue quickly"),
- student suggestions or recommendations about EduTap product/service (e.g. "I would suggest...", "It would be better if...", "Edutap should..."),
- general observations about EduTap quality, course content, or team helpfulness,
- short affirmative replies that confirm the student's satisfaction (e.g. "Yes, everything is correct", "Yes, both points are clear").

CRITICAL RULE — Never blur an entire sentence just because it contains one blurrrable sub-phrase.
If only part of a sentence needs blurring (e.g. a payment link embedded in a feedback sentence),
blur only that specific sub-phrase, not the whole sentence.
...
Only use mode "loose" for clearly non-testimonial PII fragments (phone numbers, emails, IDs).
Never use mode "loose" for feedback sentences — if you cannot match it exactly, skip it entirely.
"""
...
({ selector, phrases, blurPx }) => {
  ...
  (function blurDigitAndEmailBlocks(){
    ...
    // Never trigger fallback blur for these safe known addresses
    const SAFE_EMAILS_LC = ["feedback@edutap.co.in"];
    // Maximum own-text length for a block to be considered a leaf/small element.
    // Anything larger is a container wrapping the entire email — blurring it would
    // black out the whole card, which is the bug we are fixing.
    const MAX_LEAF_CHARS = 220;

    function isSafeText(t){
      const lower = (t || "").toLowerCase();
      return SAFE_EMAILS_LC.some(s => lower.includes(s));
    }
    ...
    // Blur the nearest non-inline ancestor, but ONLY if its total text is short
    // (i.e. it is a leaf element such as a <td>, <p>, or small <div>, not the
    // wrapper div that contains the whole email body).
    function blurNearestSmallBlock(el){
      let n = el;
      while (n && n !== root){
        if (!isInline(n)){
          const blockText = (n.textContent || "").replace(/\s+/g, " ").trim();
          if (blockText.length > MAX_LEAF_CHARS){
            // This ancestor is too large — it wraps more than just the PII token.
            // Do NOT blur it. Walk up no further (blurring a parent would be worse).
            return;
          }
          n.style.filter = `blur(${blurPx}px)`;
          return;
        }
        n = n.parentElement;
      }
    ...
    // Scan text nodes; blur nearest small block when a digit sequence or email is found.
    ...
    while (walker3.nextNode()){
      ...
      if (!t.trim()) continue;
      if (isSafeText(t)) continue;
      if (DIGIT_BLOCK_RX.test(t) || EMAIL_RX.test(t)) {
        blurNearestSmallBlock(tn.parentElement || root);
      }
    }

    // Blur images inside small blocks only (handles rasterised phone/ID signature images).
    // Skip any element whose combined text content is larger than MAX_LEAF_CHARS to
    // avoid accidentally blurring image assets that live inside the whole email wrapper.
    const allBlocks = root.querySelectorAll("*");
    allBlocks.forEach(el => {
      const txt = (el.textContent || "").replace(/\s+/g, " ");
      if (isSafeText(txt)) return;
      if (DIGIT_BLOCK_RX.test(txt) || EMAIL_RX.test(txt)) {
        if (txt.trim().length <= MAX_LEAF_CHARS) {
...
async def run_pipeline(url: str, raw_template_path: str, filename_suffix: str = "") -> dict:
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            snapped: list[str] = []
            for ptxt in final_phrases:
                dom_exact = snap_to_dom_substring(div_text, ptxt, min_ratio=0.88)
