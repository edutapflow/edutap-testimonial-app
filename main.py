...
async def compose_testimonial_graphic(
    ...
    if RENDER_QUOTE_WITH_BROWSER and "pw_context" in globals() and pw_context is not None:
        ...
        quote_x = (W - qw) // 2
        # Vertically center short testimonial text inside the available blank area
        # between the header/template title area and the student-name/star block.
        quote_y = QUOTE_TOP_Y + max(0, (available_quote_h - qh) // 2)
        base.alpha_composite(quote_img, (quote_x, quote_y))
    else:
