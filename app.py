...
def get_people_lists_cached() -> Dict[str, List[str]]:
    """Build dropdown values only from actual template image filenames.

    Sources used:
    1. local GitHub assets/Templates/... files,
    2. Supabase Storage templates/... files uploaded through the app.

    Sources intentionally NOT used:
    - people_lists.json
    - backend default people lists
    - any old fallback/static names
    """
    faculty: set[str] = set()
    support: set[str] = set()

    faculty_blocked = {
        "edutap feedback", "course feedback", "event feedback", "mentor feedback",
        "edutapfeedback", "coursefeedback", "eventfeedback", "mentorfeedback",
        "gitkeep", ".gitkeep",
    }
    support_blocked = {
        "support feedback", "supportfeedback", "support_feedback",
        "gitkeep", ".gitkeep",
    }

    # Local repo templates uploaded through GitHub website.
    for folder in ["Mentor feedback", "Event Feedback"]:
        for name in _template_image_stems_from_local(folder):
            if _valid_dropdown_template_name(name, faculty_blocked):
                faculty.add(name)

    for name in _template_image_stems_from_local("Support Feedback"):
        if _valid_dropdown_template_name(name, support_blocked):
            support.add(name)
