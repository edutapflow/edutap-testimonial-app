...
def show_sample_reference(feedback_type: str) -> None:
    """Show/download a blank template reference for the selected feedback type."""
    try:
        sample = store.get_blank_template_reference(feedback_type)
    except Exception:
        sample = None

    st.markdown("#### Blank template reference")
    st.caption("Download this blank template and share it with the designer as a reference for the required 1080 x 1080 JPG template style.")

    if not sample:
        st.info(
            "No blank template reference was found for this feedback type. "
            "Upload at least one blank/person template in Supabase Storage → templates for this feedback type."
        )
        return

    sample_bytes = sample.get("bytes") or b""
    filename = sample.get("name") or f"{feedback_type}_template_reference.jpg"
    source = sample.get("source") or "template storage"

    if sample_bytes:
        st.image(sample_bytes, width=260, caption=filename)
        st.download_button(
        ...
        st.caption(f"Source: {source} template file")
