...
def upload_template_ui() -> None:
    ...
    feedback_label = st.selectbox(
        "Feedback Type",
        list(FEEDBACK_LABELS.values()),
        key="tpl_type_label",
    )
    feedback_type = TYPE_FROM_LABEL[feedback_label]

    if feedback_type in {"edutap", "course"}:
        scope = "generic"
        st.caption("This feedback type uses one generic template.")
    elif feedback_type == "mentor":
        scope = "person"
        st.caption("Mentor templates are person-specific.")
    else:
        scope = st.radio("Template Type", ["generic", "person"], horizontal=True)

    person_name = ""
    if scope == "person":
        person_name = st.text_input("Person Name", placeholder="Example: Abhishek")

    upload = st.file_uploader("Template Image", type=["jpg", "jpeg", "png", "webp"])
    ...
    if st.button("Upload Template", type="primary"):
        try:
            ...
            if feedback_type == "edutap":
                rel = "EduTap Feedback/EduTap Feedback.jpg"
            elif feedback_type == "course":
                rel = "Course Feedback/Course Feedback.jpg"
            elif feedback_type == "event" and scope == "generic":
                rel = "Event Feedback/Event Feedback.jpg"
            elif feedback_type == "support" and scope == "generic":
                rel = "Support Feedback/Support Feedback.jpg"
            elif feedback_type == "event":
                safe = safe_template_name(person_name)
                rel = f"Event Feedback/{safe}.jpg"
                lists = load_people_lists()
                if safe not in lists.get("faculty", []):
                    lists.setdefault("faculty", []).append(safe)
                    save_people_lists(lists)
            elif feedback_type == "mentor":
                safe = safe_template_name(person_name)
                rel = f"Mentor feedback/{safe}.jpg"
                lists = load_people_lists()
                if safe not in lists.get("faculty", []):
                    lists.setdefault("faculty", []).append(safe)
                    save_people_lists(lists)
                ...
                safe = safe_template_name(person_name)
                rel = f"Support Feedback/{safe}.jpg"
                lists = load_people_lists()
                if safe not in lists.get("support", []):
                    lists.setdefault("support", []).append(safe)
                    save_people_lists(lists)
            else:
            ...
            obj = None
            try:
                obj = SimpleNamespace(filename=upload.name, file=open(tmp_path, "rb"))
                _save_uploaded_template_as_jpg(obj, str(local_path))
            finally:
                if obj is not None:
                    try:
                        obj.file.close()
