...
# Names used in Mentor / Support dropdowns.
# NewDesign no longer uses person-specific template images, so names come from
# this simple JSON config file instead of assets/Templates/... image names.
NAMES_CONFIG_FILE = Path(__file__).resolve().parent / "names_config.json"
...
def init_state() -> None:
    ...
    st.session_state.setdefault("clear_form_next_run", False)
    st.session_state.setdefault("batch_add_message", "")
...
def _valid_dropdown_template_name(name: str, blocked: set[str]) -> bool:
    """Return True only for real person/template filenames.

    Important: dropdowns must be driven only by actual template images in
    GitHub assets or Supabase Storage. Old people_lists.json/default names are
    intentionally ignored so stale names like "Rohan (Support)" do not appear.
    """
    ...
    if normalized in blocked or compact in blocked:
        return False
    if normalized.startswith("."):
        return False
    # Old fallback support names used bracket labels. These must never show unless
    # a real file is deliberately named that way, which EduTap does not want.
...
def get_people_lists_cached() -> Dict[str, List[str]]:
    """Load Mentor and Support dropdown names from names_config.json only.

    Expected file in project root:

    {
      "mentor_names": ["Name 1", "Name 2"],
      "support_member_names": ["Name 1", "Name 2"]
    }

    The returned keys stay as "faculty" and "support" because the rest of the
    existing UI already uses those keys.
    """
    ...
    config = DEFAULT_NAMES_CONFIG

    try:
        if NAMES_CONFIG_FILE.exists():
            with open(NAMES_CONFIG_FILE, "r", encoding="utf-8") as f:
                loaded = json.load(f)
            if isinstance(loaded, dict):
                config = loaded
        else:
            # Create the file automatically on local systems. On Streamlit Cloud,
            # include this file in GitHub so the names are available after deploy.
            try:
                with open(NAMES_CONFIG_FILE, "w", encoding="utf-8") as f:
                    json.dump(DEFAULT_NAMES_CONFIG, f, ensure_ascii=False, indent=2)
            except Exception:
                pass
    except Exception as exc:
        print(f"WARNING: Could not load names_config.json: {exc}")
        config = DEFAULT_NAMES_CONFIG

    return {
        "faculty": clean_names(config.get("mentor_names", [])),
        "support": clean_names(config.get("support_member_names", [])),
    }
...
def clear_create_form_after_batch_add() -> None:
    """Clear only the fields needed for the next entry; keep operator name."""
...
def upload_template_ui() -> None:
    ...
    if submitted:
        ...
        try:
            ...
            # Dropdowns are now sourced from actual template image files only.
            # Do not update people_lists.json, because old fallback names can pollute dropdowns.
...
def generate_ui() -> None:
    ...
    # Streamlit does not allow changing widget-backed session_state values
    # after those widgets have been created in the same run. So after Add to Batch
    # we set a flag, rerun, and clear fields here before rendering widgets.
    if st.session_state.get("clear_form_next_run"):
        st.session_state.clear_form_next_run = False
        clear_create_form_after_batch_add()

    if st.session_state.get("batch_add_message"):
        st.success(st.session_state.batch_add_message)
        st.session_state.batch_add_message = ""
...
def records_ui() -> None:
    ...
    if HAS_AGGRID:
        ...
        if selected_source_rows:
            action_cols = st.columns([1.4, 1.2, 4])
            with action_cols[0]:
            ...
            with action_cols[1]:
                confirm_delete = st.checkbox("Confirm delete", key="records_delete_confirm_aggrid")
            with action_cols[2]:
                if st.button(
                    f"Delete Selected ({len(selected_source_rows)})",
                    type="secondary",
                    disabled=not confirm_delete,
                    key="delete_selected_records_aggrid",
                ):
                    try:
                        delete_result = store.delete_records([r.get("id") for r in selected_source_rows], delete_images=True)
                        st.success(f"Deleted {delete_result.get('deleted_count', 0)} selected record(s).")
                        if delete_result.get("storage_errors"):
                            st.warning("Some image files could not be removed from storage, but database rows were deleted.")
                        st.cache_data.clear()
                        st.rerun()
                    except Exception as exc:
                        st.error(friendly_error_message(exc))
            ...
            with action_cols[1]:
                st.checkbox("Confirm delete", disabled=True, key="records_delete_confirm_aggrid_disabled")
            with action_cols[2]:
    ...
    if selected_rows:
        ...
        with action_cols[1]:
            confirm_delete = st.checkbox("Confirm delete", key="records_delete_confirm_fallback")
        with action_cols[2]:
            if st.button(
                f"Delete Selected ({len(selected_rows)})",
                disabled=not confirm_delete,
                key="delete_selected_records_fallback",
            ):
                try:
                    delete_result = store.delete_records([r.get("id") for r in selected_rows], delete_images=True)
                    st.success(f"Deleted {delete_result.get('deleted_count', 0)} selected record(s).")
                    if delete_result.get("storage_errors"):
                        st.warning("Some image files could not be removed from storage, but database rows were deleted.")
                    st.cache_data.clear()
        ...
        with action_cols[0]:
            st.button("Download Selected ZIP", disabled=True)
        with action_cols[1]:
            st.checkbox("Confirm delete", disabled=True, key="records_delete_confirm_fallback_disabled")
        with action_cols[2]:
            st.button("Delete Selected", disabled=True)
